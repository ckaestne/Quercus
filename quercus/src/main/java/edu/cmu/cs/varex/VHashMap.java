...
public class VHashMap<K, T> extends HashMap<K,V<? extends T>> implements VMap<K,T> {
    ...
    @Override
    public void put(FeatureExpr ctx, K key, T value) {
        put(ctx, key, V.one(value));
    }

    @Override
    public void put(FeatureExpr ctx, K key, V<? extends T> value) {
   ...
   */
    @Override
    public V<? extends T> getOrDefault(Object key, final V<? extends T> defaultValue) {
