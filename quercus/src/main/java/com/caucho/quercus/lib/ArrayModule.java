  ...
  extends AbstractQuercusModule
{
                                    ...
                                    ArrayValue values)
  {
    ...
    Iterator<EnvVar> keyIter = keys.values().iterator();
    Iterator<EnvVar> valueIter = values.values().iterator();

    ArrayValue array = new ArrayValueImpl();

    while (keyIter.hasNext() && valueIter.hasNext()) {
      array.put(keyIter.next().getOne(), valueIter.next().getOne());
    }
  ...
  public static Value array_count_values(Env env, ArrayValue array)
  {
    ...
    for (EnvVar value : array.values()) {
      if (! (value.getOne().isLongConvertible()) && ! (value.getOne() instanceof StringValue))
        env.warning("Can only count STRING and INTEGER values!");
      else {
        Value count = result.get(value.getOne()).getOne();
                                           ...
                                           Value value)
  {
    ...
    Iterator<EnvVar> iter = keyArray.getValueIterator(env);

    while (iter.hasNext()) {
      array.put(iter.next().getOne(), value.copy());
    }
                                   ...
                                   @Optional Value callbackName)
  {
    ...
    if (! callbackName.isDefault()) {
      Callable callback = callbackName.toCallable(env, VHelper.noCtx(), false);
...
//      }
                                ...
                                Value padValue)
  {
    ...
    for (long ctr = 0; ctr < size; ctr++) {
      ...
      if (padFront && ctr < size - inputSize)
        ...
        newValue = input.get(keyIterator.next()).getOne();
  ...
  public static Value array_udiff_assoc(Env env, Value[] arrays)
  {
    ...
    Callable cmp = callbackValue.toCallable(env, VHelper.noCtx(), false);
    ...
    for (Value entryKey : array.keySet()) {
      Value entryValue = array.get(entryKey).getOne();
  ...
  public static Value array_udiff_uassoc(Env env, Value[] arrays)
  {
    ...
    Callable cmpValue = callbackValue.toCallable(env, VHelper.noCtx(), false);
    ...
    Callable cmpKey = callbackKey.toCallable(env, VHelper.noCtx(), false);
    ...
    implements Comparator<LongValue>
  {
    ...
    public int compare(LongValue index1, LongValue index2)
    {
      for (int i = 0; i < _arrays.length; i++) {
        ...
        Value lValue = av.get(_rows[index1.toInt()]).getOne();
        Value rValue = av.get(_rows[index2.toInt()]).getOne();
